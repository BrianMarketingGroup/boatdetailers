...
export async function POST(req: NextRequest) {
  ...
  if (type === "contact") {
    ...
    // The BFF owns persistence: it writes the contact to Postgres AND mirrors
    // the row to the platform's Google Sheet Contact tab (resolved from
    // platform_id). Awaited + fail-open so it reliably sends but never 500s
    // the visitor.
    try {
  ...
  if (type === "apply") {
    ...
    // The BFF owns persistence now: it writes the deal to Postgres AND
    // mirrors the row to the platform's Google Sheet (resolved from
    // platform_id). Awaited + fail-open so it reliably sends but never 500s
    // the applicant.
    try {
