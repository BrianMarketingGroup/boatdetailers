export interface FaqItem {
  question: string;
  answer: string;
}

export const faqItems: FaqItem[] = [
  {
    question: "Who is BoatDetailers.com for?",
    answer:
      "BoatDetailers.com is for professional marine detailing businesses — independent detailers, detailing companies, and mobile marine service providers — who want to increase their visibility and reach boat owners actively searching for cleaning, polishing, and protection services in their area.",
  },
  {
    question: "What does a listing cost?",
    answer:
      "$289 per city for a basic listing. Featured placement — which pins your business at the top of your city page above all other listings — is an additional $689 per city. Pricing is for a 12-month annual listing.",
  },
  {
    question: "When will my listing go live?",
    answer:
      "The directory launches in August 2026. All listings debut simultaneously, so every business that applies now will be live from day one. Each listing is for 12 months from the launch date.",
  },
  {
    question: "What is a Featured Listing?",
    answer:
      "A Featured Listing pins your business at the top of search results for your city — above all other ranked listings. Only one Featured Listing is available per city, so it's first-come, first-served.",
  },
  {
    question: "Can I be listed in multiple cities?",
    answer:
      "Yes. You can add as many cities as you serve. Each city is billed separately at $289/year. Featured placement is available per city for an additional fee.",
  },
  {
    question: "What services can I highlight?",
    answer:
      "You can showcase any services your business offers — wash and wax, full detailing, yacht detailing, gel coat restoration, oxidation removal, ceramic coatings, paint correction, teak cleaning, metal polishing, interior detailing, bottom cleaning, mobile service, and more. Your specialties are featured prominently on your listing.",
  },
  {
    question: "Is my business guaranteed to be listed?",
    answer:
      "Yes — every qualifying marine detailing business that applies is accepted. We will reach out if we need any additional information to finalize your listing.",
  },
  {
    question: "What is the pre-launch special?",
    answer:
      "Businesses that apply before our August 2026 launch are eligible to receive 50% off all listings. This limited-time offer provides the same recognition, benefits, and visibility at a reduced prelaunch rate.",
  },
  {
    question: "Can I cancel my listing?",
    answer:
      "Listings are annual. Once your listing is finalized, the fee is non-refundable. You may cancel auto-renewal before the next annual cycle.",
  },
  {
    question: "Do I need to be a large company to apply?",
    answer:
      "Not at all. BoatDetailers.com welcomes solo detailers, mobile operators, and full-service detailing companies alike. If you provide professional marine detailing services, you qualify for a listing.",
  },
  {
    question: "What types of vessels can I list specialties for?",
    answer:
      "You can highlight any vessel types you service — center consoles, sportfishing boats, cruisers, yachts, sailboats, pontoons, personal watercraft, and more. Your profile communicates exactly what you specialize in.",
  },
];

export const howItWorksFaqItems: FaqItem[] = [
  faqItems[0],
  faqItems[2],
  faqItems[3],
  faqItems[6],
];
